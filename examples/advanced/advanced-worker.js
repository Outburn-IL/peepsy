...
worker.registerHandler('generateReport', async (data) => {
  ...
  const report = {
    ...
    summary: {
      ...
      successRate: data.totalTasks > 0 ? (data.completedTasks / data.totalTasks * 100).toFixed(2) + '%' : '0%',
      uptime: formatDuration(data.uptime),
      averageTasksPerSecond: data.uptime > 0 ? (data.totalTasks / (data.uptime / 1000)).toFixed(2) : '0'
    },
...
function generateRecommendations(data) {
  ...
  if (avgTasksPerSecond < 1) {
    recommendations.push('Low throughput detected. Consider optimizing task processing or scaling workers.');
  }
...
setInterval(async () => {
  if (!currentTask && Math.random() > 0.7) { // 30% chance to request task
    try {
      ...
      if (task) {
        ...
        const action = task.type === 'cpu-intensive' ? 'processCpuTask' : 'processIoTask';
